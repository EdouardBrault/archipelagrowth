...
const ContactFormSection = ({ config }: ContactFormSectionProps) => {
  const { t } = useLanguage();
  const cardTitle = config?.cardTitle || t.contactForm.cardTitle;
  const formTitle = config?.formTitle || t.contactForm.formTitle;
  const formDescription = config?.formDescription || t.contactForm.formDescription;

  const { sendFormData } = useFormWebhook();
  const { toast } = useToast();
  const { getUtmParams } = useUtmTracking();
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [formData, setFormData] = useState({ lastName: "", firstName: "", phone: "", email: "", company: "", requestType: t.contactForm.requestTypes[0], message: "" });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await sendFormData({ form_name: "Contact", form_id: "contact-form", fields: { nom: formData.lastName, prenom: formData.firstName, telephone: formData.phone, email: formData.email, entreprise: formData.company, demande: formData.requestType, message: formData.message }, url: window.location.href, utm_params: utmParams });
      toast({ title: t.contactForm.successTitle, description: t.contactForm.successDescription });
      setFormData({ lastName: "", firstName: "", phone: "", email: "", company: "", requestType: t.contactForm.requestTypes[0], message: "" });
    } catch {
          ...
          <div>
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-[#010D3E] font-inter mb-1">{t.contactForm.firstName} <span className="text-red-500">*</span></label>
                ...
                <div>
                  <label className="block text-sm font-medium text-[#010D3E] font-inter mb-1">{t.contactForm.lastName} <span className="text-red-500">*</span></label>
              ...
              <div>
                <label className="block text-sm font-medium text-[#010D3E] font-inter mb-1">{t.contactForm.phone} <span className="text-red-500">*</span></label>
              ...
              <div>
                <label className="block text-sm font-medium text-[#010D3E] font-inter mb-1">{t.contactForm.email} <span className="text-red-500">*</span></label>
              ...
              <div>
                <label className="block text-sm font-medium text-[#010D3E] font-inter mb-1">{t.contactForm.company} <span className="text-red-500">*</span></label>
              ...
              <div>
                <label className="block text-sm font-medium text-[#010D3E] font-inter mb-1">{t.contactForm.requestAbout}</label>
                <div className="flex flex-wrap items-center gap-3 mt-2">
                  {t.contactForm.requestTypes.map((d) => (
                    <button key={d} type="button" onClick={() => handleChange("requestType", d)} className={`px-5 py-2 rounded-full border text-sm font-inter transition-all ${formData.requestType === d ? "border-[#0043F1] bg-[#0043F1] text-white" : "border-[#0043F1] text-[#0043F1] bg-white hover:bg-[#0043F1]/5"}`}>{d}</button>
                  ))}
                </div>
              </div>
              <div>
                <label className="block text-sm font-medium text-[#010D3E] font-inter mb-1">{t.contactForm.message}</label>
              ...
              <div className="flex justify-end">
                <Button type="submit" disabled={isSubmitting} className="bg-[#0043F1] text-white hover:bg-[#0043F1]/90 font-inter font-normal px-8 py-3 rounded-full">
                  {isSubmitting ? (<><Loader2 className="mr-2 w-4 h-4 animate-spin" />{t.contactForm.sending}</>) : (<>{t.contactForm.send}<ArrowRight className="ml-2 w-4 h-4" /></>)}
                </Button>
              </div>
            </form>
          </div>
